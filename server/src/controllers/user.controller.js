...
import { deleteFileFromCloud, uploadFileOnCloud } from "../utils/fileUpload.js";
...
const generateAccessAndRefreshToken = async (userId) => {
  try {
    ...
    console.log("this is the user.refreshToken", user.refreshToken);
...
const logoutUser = asyncHandler(async (req, res) => {
  await User.findOneAndUpdate(req.body.refreshToken, { refreshToken: "" });
...
const refreshAccessToken = asyncHandler(async (req, res) => {
  const incomigRefreshToken =
    req.cookies.refreshToken || req.params.refreshToken || req.query.refreshToken || req.body.refreshToken;
  ...
  try {
    ...
    if (incomigRefreshToken !== user?.refreshToken) throw new ApiError(401, "Refresh token is expired or used");
