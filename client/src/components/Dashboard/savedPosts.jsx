    ...
    <>
      {displayedSavedPosts.map((postData) => (
        <div className="m-2 md:w-96 md:m-4" key={postData._id}>
          <div className="flex">
            <p className="text-slate-500 text-sm md:text-base px-0.5">Blogger Hawk</p>
          </div>
          <div onClick={() => handleClick(postData._id)} className="flex items-center cursor-pointer select-none">
            <div>
              <h2 className="font-Andada Pro text-base font-medium">{postData.title}</h2>
            </div>
          </div>
        </div>
      ))}
      {savedPosts.length > 3 && (
        <button className="px-2 mx-2 text-lg text-blue-500" onClick={() => setShowAllSavedPosts(!showAllSavedPosts)}>
